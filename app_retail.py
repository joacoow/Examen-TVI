...
<style>
    ...
    /* 6. Estilo para las Pestañas (Tabs tipo Dash a Ancho Completo) */
    ...
    .stTabs [aria-selected="true"] {
        background-color: rgba(29,111,164,0.07) !important;
        border: 1px solid #e0e6ed !important;
        border-bottom: 3px solid #1d6fa4 !important;
        color: #1d6fa4 !important;
...
with tab_mapa:
    ...
    fig_mapa = px.choropleth_mapbox(
        ...
        center={"lat": 45.0, "lon": 0.0}, # Centrado ajustado a Europa/Atlántico */
        opacity=0.85
    )
    ...
    fig_mapa.update_layout(
        ...
        # 🚨 Reducimos el margen inferior (b=90) para eliminar el gran espacio en blanco
    ...
    # 🚨 Subimos los controles cambiando la coordenada Y
    if fig_mapa.layout.updatemenus:
        fig_mapa.layout.updatemenus[0].x = 0.05
        fig_mapa.layout.updatemenus[0].y = -0.0 # Aceramos el botón hacia el mapa
        
    if fig_mapa.layout.sliders:
        fig_mapa.layout.sliders[0].x = 0.08
        fig_mapa.layout.sliders[0].y = 0#Acercamos la barra hacia el mapa
...
with tab_relaciones:
    ...
    fig_pc.update_layout(
        ...
        margin=dict(l=70, r=70, t=40, b=40), height=550
    )
...
with tab_polar:
    ...
    if 'InvoiceDate' in dff.columns:
        ...
        fig_polar.update_traces(fill='toself', hovertemplate="<b>%{theta}</b><br>Segmento: %{trace.name}<br>Facturación: £%{r:,.0f}<extra></extra>")
        fig_polar.update_layout(
            LAYOUT, margin=dict(l=60, r=160, t=40, b=40), height=650,
            polar=dict(
                bgcolor="#eef1f7", domain=dict(x=[0, 0.82]),
            ...
            legend=dict(title="Mercado", orientation="v", yanchor="middle", y=0.5, xanchor="left", x=0.85, bgcolor="rgba(0,0,0,0)", font=dict(color=TXT_COLOR))
        )
